...
def detalle_pedido(request, pk):
    ...
    productos = ProductoInventario.objects.all()  # Para el modal de agregar
        ...
        if accion == 'agregar_producto':
            ...
            if producto_id and precio:
                try:
                    ...
                    detalle_existente = DetallePedidoProveedor.objects.filter(pedido_proveedor=pedido, producto_inventario=producto).first()
                    
                    if detalle_existente:
                        # Actualizar cantidad y precio si ya existe
                        detalle_existente.cantidad += int(cantidad)
                        detalle_existente.precio_unitario = Decimal(precio)
                        # Actualizar subtotal
                        detalle_existente.subtotal = detalle_existente.cantidad * detalle_existente.precio_unitario
                        detalle_existente.save()
                        messages.success(request, f'Se actualizó la cantidad del producto {producto.nombre_producto}')
                    else:
                        ...
                        messages.success(request, f'Producto {producto.nombre_producto} agregado al pedido')
                    
                    # Actualizar el precio total si existe un método para ello
                    if hasattr(pedido, 'calcular_total'):
                        pedido.calcular_total()
                        pedido.save()
                    
                except (ProductoInventario.DoesNotExist, ValueError, Exception) as e:
                    messages.error(request, f'Error al agregar el producto: {str(e)}')
            else:
                messages.error(request, 'Producto y precio son requeridos')
...
def editar_producto_pedido(request, pedido_id):
    """
    Vista para editar un producto en un pedido existente.
    """
    if request.method == 'POST':
        ...
        try:
            detalle = DetallePedidoProveedor.objects.get(pk=detalle_id)
            
            # Verificar que el detalle pertenece al pedido indicado
            if detalle.pedido_proveedor.id != int(pedido_id):
                messages.error(request, 'Error: El producto no pertenece a este pedido')
                return redirect('inventario:detalle_pedido', pk=pedido_id)
            ...
            # Actualizar el total del pedido si existe el método
            ...
            messages.error(request, 'Error: No se encontró el producto en el pedido')
...
def crear_ubicacion(request, almacen_id):
    """Vista para crear una nueva ubicación en un almacén"""
    almacen = get_object_or_404(Almacen, pk=almacen_id)
    
    if request.method == 'POST':
        form = UbicacionAlmacenForm(request.POST)
        if form.is_valid():
            # Crear pero no guardar todavía
            ubicacion = form.save(commit=False)
            # Asignar el almacén manualmente
        ...
        # Al crear un nuevo formulario, no necesitamos initial ya que el campo almacen no está en el formulario
        form = UbicacionAlmacenForm()
...
def asignar_producto_ubicacion(request, ubicacion_id):
    """Vista para asignar productos a una ubicación específica de almacén"""
    ...
    if request.method == 'POST':
        ...
        print(f"DEBUG: Datos del formulario: {request.POST}")
        print(f"DEBUG: Formulario válido: {form.is_valid()}")
        if form.is_valid():
            print("DEBUG: El formulario es válido")
            inventario = form.save(commit=False)
            inventario.ubicacion_almacen = ubicacion  # Asignar la ubicación manualmente
            print(f"DEBUG: Ubicación asignada: {ubicacion.id} - {ubicacion.nombre_ubicacion}")
            ...
            if existente:
                print(f"DEBUG: Producto existente encontrado, actualizando cantidad: {existente.id}")
                existente.cantidad += inventario.cantidad  # Actualizado de cantidad_disponible a cantidad
                existente.save()
                messages.success(request, f'Cantidad actualizada para {inventario.producto_inventario.nombre_producto}.')
            else:
                print("DEBUG: Creando nueva asignación de producto")
                inventario.save()
                print(f"DEBUG: Asignación guardada con ID: {inventario.id}")
